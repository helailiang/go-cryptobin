...
func namedCurveFromOID(oid asn1.ObjectIdentifier) elliptic.Curve {
	...
	case oid.Equal(secp256k1.OIDNamedCurveSecp256k1):
		return secp256k1.S256() //k1
...
func oidFromNamedCurve(curve elliptic.Curve) (asn1.ObjectIdentifier, bool) {
	...
	case secp256k1.S256(), ethcrypto.S256():
		return secp256k1.OIDNamedCurveSecp256k1, true // k1
	case sm2.P256():
...
func signingParamsForPublicKey(pub any, requestedSigAlgo SignatureAlgorithm) (hashFunc Hash, sigAlgo pkix.AlgorithmIdentifier, err error) {
	...
	case *ecdsa.PublicKey:
		...
		case secp256k1.S256(), ethcrypto.S256():
			hashFunc = SHA256
			sigAlgo.Algorithm = secp256k1.OIDNamedCurveSecp256k1
		default:
...
func FromX509Certificate(x509Cert *x509.Certificate) *Certificate {
	c := &Certificate{}
	c.Raw = x509Cert.Raw
	c.RawTBSCertificate = x509Cert.RawTBSCertificate
	c.RawSubjectPublicKeyInfo = x509Cert.RawSubjectPublicKeyInfo
	...
	c.RawIssuer = x509Cert.RawIssuer
	c.Signature = x509Cert.Signature
	c.SignatureAlgorithm = SignatureAlgorithm(x509Cert.SignatureAlgorithm) //SM2WithSM3
	c.PublicKeyAlgorithm = PublicKeyAlgorithm(x509Cert.PublicKeyAlgorithm)
	c.PublicKey = x509Cert.PublicKey
	c.Version = x509Cert.Version
	c.SerialNumber = x509Cert.SerialNumber
...
func FromX509CertificateRequest(x509Cert *x509.CertificateRequest) *CertificateRequest {
	c := &CertificateRequest{}
	err := copier.Copy(c, x509Cert)
	if err != nil {
		fmt.Printf("copier.FromX509CertificateRequest error:%s", err)
	}
	return c
}
...
// converts a public key to PKIX, ASN.1 DER 将公钥转为der格式
